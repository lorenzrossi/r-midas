#!/usr/bin/env Rscript
# =============================================================================
# v2 PIPELINE — SUMMARY TABLE (CORE FAMILIES ONLY)
#
# Same metrics and layout as summary_table.R, but restricted to:
#   - AR            (ar_<cc>_h<h>.csv)
#   - RU_MIDAS      (ru_midas_<cc>_h<h>.csv)
#   - R_MIDAS       (r_midas_<cc>_h<h>.csv) — standard, Almon on y only
#
# Excludes R_MIDAS_extended.  Use this for interim results while the full
# pipeline is still running extended R-MIDAS.
#
# Outputs (separate from the full summary so run_all.R can still write the
# complete table later):
#   results/summary_table_core.csv
#   results/summary_top5_core_per_country_h.csv
#
# Missing forecast CSVs are skipped silently (e.g. r_midas_it_* until Italy
# finishes).  AR files must exist for each (country, h) block processed.
#
# Paths: V2_DATA_DIR or V2_SCRIPT_DIR, same as summary_table.R / v2_paths.R.
# =============================================================================
...
# --- metric helpers (identical to summary_table.R) --------------------------------
...
compute_metrics <- function(y_actual, y_hat_test, y_hat_bench, h) {
  ...
  sig2_t <- mean(e_t_a^2)
  sig2_b <- mean(e_b_a^2)
  y_t_bc <- y_t * (1 + sig2_t / 2)
  y_b_bc <- y_b * (1 + sig2_b / 2)
  e_t_bc <- y_a - y_t_bc
  e_b_bc <- y_a - y_b_bc
...
# Core families only — no R_MIDAS_extended
...
out_path <- file.path(RESULT_DIR, "summary_table_core.csv")
...
best_path <- file.path(RESULT_DIR, "summary_top5_core_per_country_h.csv")
