#!/usr/bin/env Rscript
# =============================================================================
# v2 PIPELINE — R-MIDAS LEGACY (GERMANY-ONLY, 10-YEAR WINDOW, RAW VARIABLES)
#                                                              (PARALLEL)
#
# ASYMMETRIC-WINDOW VARIANT — see header in r_midas_legacy_short_it.R for the
# matching Italy script.  This file uses a 10-year (3650-day) rolling window
# on the FULL 2005-2025 sample so that the first feasible forecast origin
# falls around mid-January 2015.  Italy uses a 2-year (730-day) window on a
# 2013-2025 sub-sample to land at the SAME first forecast origin, giving the
# two countries the same evaluation period (2015-2025) at the cost of
# methodologically asymmetric window lengths.  This is a deliberate
# compromise — defensible only if reported clearly in any writeup.
#
# Germany twin of r_midas_legacy_short_it.R.  Same conventions: raw variables
# (no asinh / log / Δlog / scale), 256-spec combo grid, 5-start L-BFGS-B
# (maxit=3000, factr=1e7), exp-Almon on Q=14 daily AR lags of y with linear
# HF lag1 / LF / calendar blocks, per-spec parallel, in-window position
# decomposition of LF coefficients.
#
# Differences from r_midas_legacy_short_it.R: country/cc/LF candidates, and
# the daily / monthly file names.
#
# NOTE on raw y for Germany.  Daily DE prices include 14 negative values in
# 2005-2025.  Running the model in raw EUR/MWh handles these natively (the
# AR-Almon block is linear in y).  Keep this in mind when interpreting
# coefficients — they're in raw EUR/MWh per unit of regressor.
#
# OUTPUTS:
#   forecasts/r_midas_legacy_short_de_h<h>.csv
#   forecasts/r_midas_legacy_short_de_spec_dictionary.csv
#   results/summary_legacy_short_de.csv
#   results/summary_legacy_short_de_top5.csv
#   results/coefficients/r_midas_legacy_short_de.csv
#   results/position_decomposition_de.csv     (28 mean λ_i per LF per spec)
# =============================================================================
...
WINDOW_DAYS_SHORT <- 3650L     # 10-year window — asymmetric variant (DE long, IT short)
Q_AR              <- 14L
K_POS             <- 28L
...
country  <- "Germany"
cc       <- "de"
lf_names <- c("pmi_mfg_de", "pmi_serv_de", "ifo_bci_de",
              "Consumer_Goods", "Manufacturing", "Energy")

# Load DE data — RAW
daily_path   <- file.path(DATA_DIR, "prices_de_daily.csv")
monthly_path <- file.path(DATA_DIR, "dataset_de_m.csv")
...
cat(sprintf("y range: [%.2f, %.2f]   mean=%.2f   n=%d   (%d negative obs)\n",
            min(y, na.rm=TRUE), max(y, na.rm=TRUE), mean(y, na.rm=TRUE),
            n, sum(y < 0, na.rm=TRUE)))
...
dict_path <- file.path(FORECAST_DIR, "r_midas_legacy_short_de_spec_dictionary.csv")
...
cat(sprintf("rolling origins: %d (from %s to %s)\n",
            n_orig, format(dates[origins[1]]), format(dates[origins[n_orig]])))

# Legacy NLS helpers (inline, identical to r_midas_legacy_short_it.R)
...
n_cores <- get_n_cores()
cat(sprintf("\n=== DE short-window legacy run  (n_cores=%d, n_starts=%d, window=%d) ===\n",
...
for (h in HORIZONS) {
  ...
  fp <- file.path(FORECAST_DIR, sprintf("r_midas_legacy_short_de_h%02d.csv", h))
...
coef_path <- file.path(COEF_DIR, "r_midas_legacy_short_de.csv")
...
if (length(pos_rows) > 0L) {
  ...
  pos_path <- file.path(RESULT_DIR, "position_decomposition_de.csv")
...
.rmse_block <- function(e_t, e_b, h) {
  ...
  list(rmse=rmse, rmse_se=rmse_se, rmse_ratio=rmse_ratio, dm_stat=dm$stat, dm_pval=dm$pvalue)
}
.mae_block <- function(e_t, e_b, h) {
  ...
  list(mae=mae, mae_se=mae_se, mae_ratio=mae_ratio, dm_stat=dm$stat, dm_pval=dm$pvalue)
}
compute_metrics_local <- function(y_actual, y_hat_test, y_hat_bench, h) {
  ...
  # asinh side — NB: y on raw scale, so asinh(y) is just a numerical transform here
...
for (h in HORIZONS) {
  ...
  leg_path <- file.path(FORECAST_DIR, sprintf("r_midas_legacy_short_de_h%02d.csv", h))
...
if (length(sum_rows) > 0L) {
  ...
  sum_path <- file.path(RESULT_DIR, "summary_legacy_short_de.csv")
  ...
  top_path <- file.path(RESULT_DIR, "summary_legacy_short_de_top5.csv")
  write.csv(top5, top_path, row.names = FALSE)
  cat(sprintf("wrote %s  (rows = %d)\n", top_path, nrow(top5)))
}
cat("\nR-MIDAS legacy short-window DE done.\n")
