...
# =============================================================================
# Plot low-frequency coefficients over time (Italy — screenshot models)
#
# Same layout as plot_lf_coefficients_de.R.
# Default inputs:
#   - results/coefficients/r_midas_it.csv
#   - forecasts/r_midas_spec_dictionary_it.csv
#
# Override with env vars:
#   COEF_FILE, SPEC_DICT_FILE, OUT_DIR, SUMMARY_FILE
# =============================================================================
...
coef_file <- Sys.getenv(
  "COEF_FILE",
  unset = file.path(script_root, "results", "coefficients", "r_midas_it.csv")
)
dict_file <- Sys.getenv(
  "SPEC_DICT_FILE",
  unset = file.path(script_root, "forecasts", "r_midas_spec_dictionary_it.csv")
)
out_dir <- Sys.getenv(
  "OUT_DIR",
  unset = file.path(script_root, "results", "plots", "lf_coefficients_it")
)
...
model_defs <- list(
  list(
    ...
    hf_vars = "gas",
    lf_vars = "istat_bci_it"
  ),
  list(
    label = "R-MIDAS + gas + IPI C.Goods + IPI Energy + ISTAT BCI",
    hf_vars = "gas",
    lf_vars = "istat_bci_it|Consumer_Goods|Energy"
  )
)
...
resolve_spec <- function(hf_vars, lf_vars) {
  ...
  if (length(idx) > 1L) sort(specs$spec_id[idx])[1] else specs$spec_id[idx]
}
...
for (k in seq_along(model_defs)) {
  ...
  matplot(
    ...
    main = "R-MIDAS LF coefficients (Italy)"
  )
...
map_path <- file.path(out_dir, "model_to_spec_map_it.csv")
