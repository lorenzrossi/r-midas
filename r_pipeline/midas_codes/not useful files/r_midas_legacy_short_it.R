#!/usr/bin/env Rscript
# =============================================================================
# v2 PIPELINE — R-MIDAS LEGACY (ITALY-ONLY, 2-YEAR WINDOW, RAW VARIABLES)
#                                                              (PARALLEL)
#
# ASYMMETRIC-WINDOW VARIANT — see header in r_midas_legacy_short_de.R for the
# matching Germany script.  This file uses a 2-year (730-day) rolling window
# but FILTERS the daily Italy data to start at 2013-01-01, so the first
# feasible forecast origin lands around mid-January 2015.  Germany uses a
# 10-year (3650-day) window on the full 2005-2025 sample to land at the same
# first forecast origin, giving both countries the same evaluation period
# (2015-2025) at the cost of methodologically asymmetric window lengths.
# Reported clearly in any writeup.
#
# Italy-only port of the legacy r_midas_surveys_h.R script, run on the new
# (filtered) 2013-2025 daily dataset.
#
# Key differences from r_midas_legacy.R:
#   1) WINDOW_DAYS_SHORT = 730L                (was 3650L)
#   2) ITALY ONLY                              (no Germany)
#   3) ALL VARIABLES IN RAW UNITS              (was asinh/log/Δlog/z-score)
#         y         = elec_price                (was asinh(elec_price))
#         gas_lag1  = gas_price[t-1]            (was log gas_price)
#         brent_lag1= brent_price[t-1]          (was log brent_price)
#         IPI       = raw level                 (was Δlog)
#         PMI/ISTAT = raw                       (was scale())
#   4) FORECASTS ON €/MWh DIRECTLY             (no sinh back-transform)
#
# Same as r_midas_legacy.R:
#   * 256-spec combo grid over {gas, brent} × {pmi_mfg_it, pmi_serv_it,
#     istat_bci_it, Consumer_Goods, Manufacturing, Energy}
#   * exp-Almon on Q=14 daily AR lags of y; gas_lag1 / brent_lag1 / LF
#     enter linearly when included
#   * 18-column calendar block (6 weekday + 11 month + 1 holiday)
#   * 5-start L-BFGS-B per rolling window with warm-start θ; maxit=3000,
#     factr=1e7 (legacy defaults)
#   * Iterated multi-step at h ∈ {1, 7, 14, 21, 28} (option c.1)
#   * Per-spec parallel via parallel_lapply
#
# OUTPUTS:
#   forecasts/r_midas_legacy_short_it_h<h>.csv
#   forecasts/r_midas_legacy_short_it_spec_dictionary.csv
#   results/summary_legacy_short_it.csv               unified summary (this scale)
#   results/summary_legacy_short_it_top5.csv          best 5 per horizon
#   results/coefficients/r_midas_legacy_short_it.csv  rolling pooled λ time series
#   results/position_decomposition_it.csv             28 mean λ_i per LF per spec
#                                                     averaged across rolling origins,
#                                                     to characterise within-month
#                                                     variation in LF informativeness
#
# Position decomposition: at every rolling origin, after the pooled NLS
# converges, we re-run a SECOND OLS on the same window that interacts every
# LF variable with 28 within-month-position dummies (days where pos > 28
# are dropped, Cimadomo convention).  This second OLS uses the θ_y from the
# pooled NLS so the AR-Almon block is identical; the only change is that the
# LF block has 28 coefficients per variable instead of 1.  The 28 fitted λ_i
# values per origin are accumulated into running sums and counts, and at the
# end we report the mean across rolling origins.
# =============================================================================

# -----------------------------------------------------------------------------
# Bootstrap — robust path resolution (matches summary_table.R convention)
# -----------------------------------------------------------------------------
code_dir <- tryCatch({
...
# Constants
# -----------------------------------------------------------------------------
WINDOW_DAYS_SHORT <- 730L
Q_AR              <- 14L
K_POS             <- 28L              # within-month positions
...
country  <- "Italy"
cc       <- "it"
lf_names <- c("pmi_mfg_it", "pmi_serv_it", "istat_bci_it",
              "Consumer_Goods", "Manufacturing", "Energy")

# -----------------------------------------------------------------------------
# Load Italy data — RAW values (no transformations)
# -----------------------------------------------------------------------------
daily_path   <- file.path(DATA_DIR, "prices_it_daily.csv")
monthly_path <- file.path(DATA_DIR, "dataset_it_m.csv")
...
# ASYMMETRIC-WINDOW VARIANT: trim Italy daily data to 2013-01-01 onwards so
# that the first feasible forecast origin (data_start + 730 + 14 days) lands
# in mid-January 2015 — matching the Germany script's first origin under a
# 10-year window on the full 2005-2025 sample.  Monthly data is left intact
# because the LF block is publication-aware LOCF and tolerates a wider
# monthly history.
IT_DATA_START <- as.Date("2013-01-01")
daily <- daily[daily$date >= IT_DATA_START, , drop = FALSE]
cat(sprintf("Italy daily data filtered to >= %s : %d rows remaining\n",
            format(IT_DATA_START), nrow(daily)))

dates <- daily$date
n     <- nrow(daily)
y     <- as.numeric(daily$elec_price)         # RAW EUR/MWh
gas_lag1   <- c(NA_real_, as.numeric(daily$gas_price)[1:(n - 1L)])      # RAW
brent_lag1 <- c(NA_real_, as.numeric(daily$brent_price)[1:(n - 1L)])    # RAW

cat(sprintf("y range: [%.2f, %.2f]   mean=%.2f   n=%d\n",
            min(y, na.rm=TRUE), max(y, na.rm=TRUE), mean(y, na.rm=TRUE), n))

# AR lag matrix on RAW y
...
# LF (publication-aware LOCF, RAW values)
lf_pub <- c(pmi_mfg_it = PUB_LAG_SURVEY, pmi_serv_it = PUB_LAG_SURVEY,
...
# Calendar dummies (6 weekday + 11 month + 1 holiday)
...
dict_path <- file.path(FORECAST_DIR, "r_midas_legacy_short_it_spec_dictionary.csv")
write.csv(dict, dict_path, row.names = FALSE)
cat(sprintf("wrote %s\n", dict_path))

# Rolling origins on the SHORT window
origins <- make_origins(n, WINDOW_DAYS_SHORT,
...
# -----------------------------------------------------------------------------
# Per-spec worker — pooled NLS forecasts + position decomposition (OLS)
# -----------------------------------------------------------------------------
...
fit_one_spec <- function(si) {
  ...
  # Position decomposition: running sums and counts (only for LFs in spec)
  ...
  for (oi in seq_along(origins)) {
    t0 <- origins[oi]
    win_lo <- t0 - WINDOW_DAYS_SHORT + 1L
    win_rows <- win_lo:t0
    ywin <- y[win_rows]
    ar_lags_win <- ar_lag_mat[win_rows, , drop = FALSE]

    # Linear design for pooled NLS
    ...
    # ---- POSITION DECOMPOSITION (OLS, using θ_y from the pooled NLS) ----
    if (length(lf_used) > 0L) {
      w_y <- fit$weights
      z_y_win <- as.numeric(ar_lags_win %*% w_y)
      pos_win <- within_month_pos[win_rows]
      # Build LF × position-dummy columns
      ...
      # HF lag1 if in spec (pooled, NOT position-interacted)
      hf_blk <- list()
      if ("gas" %in% hf_used)
        hf_blk[["gas_lag1"]] <- gas_lag1[win_rows]
      if ("brent" %in% hf_used)
        hf_blk[["brent_lag1"]] <- brent_lag1[win_rows]

      X_pos <- cbind(X_base[win_rows, , drop = FALSE],
                     z_y = z_y_win,
    ...
    # ---- Frozen linear row at (origin + 1) for iterated forecast ----
...
# -----------------------------------------------------------------------------
n_cores <- get_n_cores()
cat(sprintf("\n=== IT short-window legacy run  (n_cores=%d, n_starts=%d, window=%d) ===\n",
            n_cores, optim_starts, WINDOW_DAYS_SHORT))
t_start <- Sys.time()
spec_results <- parallel_lapply(seq_along(spec_ids), fit_one_spec, n_cores = n_cores)
el <- as.numeric(difftime(Sys.time(), t_start, units = "secs"))
cat(sprintf("fitting done in %.1fs (%.2fh)\n", el, el / 3600))

# -----------------------------------------------------------------------------
# Write forecast CSVs (no sinh() — y is already on €/MWh)
# -----------------------------------------------------------------------------
for (h in HORIZONS) {
  ...
  fp  <- file.path(FORECAST_DIR,
                   sprintf("r_midas_legacy_short_it_h%02d.csv", h))
  write.csv(out, fp, row.names = FALSE)
  cat(sprintf("wrote %s  (rows = %d)\n", fp, nrow(out)))
}

# -----------------------------------------------------------------------------
# Pooled rolling λ time series CSV (origin_date × spec_id × LF)
# -----------------------------------------------------------------------------
...
coef_path <- file.path(COEF_DIR, "r_midas_legacy_short_it.csv")
write.csv(coef_df, coef_path, row.names = FALSE)
cat(sprintf("wrote %s  (rows = %d)\n", coef_path, nrow(coef_df)))

# -----------------------------------------------------------------------------
# Position decomposition CSV — 28 mean λ_i per LF per spec
# (long format: spec_id, lf_var, position, mean_lambda, n_origins_used)
# -----------------------------------------------------------------------------
pos_rows <- list()
for (si in seq_along(spec_ids)) {
  r <- spec_results[[si]]
  ...
  for (v in r$lf_used) {
    for (i in 1:K_POS) {
      cnt <- r$pos_count[v, i]
      mean_lam <- if (cnt > 0L) r$pos_sum[v, i] / cnt else NA_real_
...
if (length(pos_rows) > 0L) {
  ...
  pos_path <- file.path(RESULT_DIR, "position_decomposition_it.csv")
  write.csv(pos_df, pos_path, row.names = FALSE)
  cat(sprintf("wrote %s  (rows = %d, ≈ specs×LF×28)\n", pos_path, nrow(pos_df)))
} else {
  cat("no position decomposition rows (no spec had LF variables)\n")
}

# -----------------------------------------------------------------------------
# Summary table — same metric block as summary_table.R, restricted to IT and
# this family.  Uses the existing AR forecast files (ar_it_h<h>.csv) as the
# benchmark.  N.B. — the AR forecasts in this pipeline are on the asinh scale,
# back-transformed; the short legacy forecasts here are on RAW €/MWh.  So the
# €/MWh rmse_ratio is the right thing to read here.  rmse_asinh / mae_asinh
# columns are computed for completeness but are NOT directly comparable
# across the two scales of y treatment — interpret them with care.
# -----------------------------------------------------------------------------
.rmse_block <- function(e_t, e_b, h) {
  n_ <- length(e_t)
  if (n_ < 5L) return(list(rmse = NA, rmse_se = NA, rmse_ratio = NA,
                           dm_stat = NA, dm_pval = NA))
...
.mae_block <- function(e_t, e_b, h) {
  n_ <- length(e_t)
  if (n_ < 5L) return(list(mae = NA, mae_se = NA, mae_ratio = NA,
                           dm_stat = NA, dm_pval = NA))
...
sum_rows <- list()
for (h in HORIZONS) {
  ar_path  <- file.path(FORECAST_DIR, sprintf("ar_it_h%02d.csv", h))
  leg_path <- file.path(FORECAST_DIR, sprintf("r_midas_legacy_short_it_h%02d.csv", h))
  if (!file.exists(ar_path) || !file.exists(leg_path)) {
    cat(sprintf("[skip] h=%02d : missing forecast file\n", h)); next
  }
  ...
  cat(sprintf("[ok] h=%02d : %d specs, n=%d common dates\n",
              h, length(spec_cols_h), length(common)))
...
if (length(sum_rows) > 0L) {
  ...
  # Attach hf_vars / lf_vars from the dictionary we wrote
  ...
  top_path <- file.path(RESULT_DIR, "summary_legacy_short_it_top5.csv")
  write.csv(top5, top_path, row.names = FALSE)
  cat(sprintf("wrote %s  (rows = %d)\n", top_path, nrow(top5)))
} else {
  cat("no summary rows produced — check AR benchmark files exist for IT.\n")
}

cat("\nR-MIDAS legacy short-window IT done.\n")
